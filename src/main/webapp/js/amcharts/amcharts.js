    ...
    d.AxisBase = d.Class({
        construct: function (a) {
            ...
            if (0 < this.gridCountR) for (B - 5 - k > this.autoRotateCount &&
                ...
                } else this.rotate ? "date" == this.type && "middle" == this.gridPosition && (K = -y * this.stepWidth /
...
(function () {
    ...
    d.RecItem = d.Class({
        construct: function (a, b, c, e, h, f, g, k, l, m, p, q) {
            ...
            g ? (void 0 !== g.id && (q = n.classNamePrefix + "-guide-" + g.id), P = !0, isNaN(g.tickLength) || (B = g.tickLength), void 0 != g.lineColor && (K = g.lineColor), void 0 != g.color && (w = g.color), isNaN(g.lineAlpha) || (D = g.lineAlpha), isNaN(g.dashLength) || (H = g.dashLength), isNaN(g.lineThickness) || (C = g.lineThickness), !0 === g.inside && (I = !0, 0 < ea && (ea = 0)), isNaN(g.labelRotation) ||
            ...
            if ("H" == Pa) {
                if (0 <= b && b <= xa + 1 && (0 < B && 0 < y && b + l <= xa + 1 && (fa = d.line(z, [b + l, b + l], [0, B], A, y, C), x.push(fa)), 0 < D && (Ba = d.line(z, [b, b + ma, b + ma], [X, X + na, na], K, D, C, H), x.push(Ba))), v = 0, G = b, g && 90 == p && I && (G -= r), !1 === e ? (ga = "start", v = "bottom" == Z ? I ? v + B : v - B : I ? v - B : v + B, G += 3, 0 < h && (G += h / 2 - 3, ga = "middle"), 0 < p && (ga = "middle")) : ga = "middle", 1 == ia && 0 < M && !g && !m && da < xa && (e = d.fitToBounds(b, 0, xa), da = d.fitToBounds(da, 0, xa), Y = e - da, 0 < Y && (Da = d.rect(z, Y, a.height, Q, M), Da.translate(e - Y + ma, na), x.push(Da))), "bottom" == Z ? (v += X + r / 2 + ea, I ? (0 < p ? (v =
                    ...
                    r + 3 + u + 3, v -= aa)), "bottom" == Z ? Y = (I ? X - B - 1 : X + u - 1) + ea : (oa = ma, Y = (I ? na : na - B - u + 1) - ea), f && (G += f), r = G, 0 < p && (r += ha / 2 * Math.cos(V)), ca && (f = 0, I && (f = ha / 2 * Math.cos(V)), r + f > xa + 2 || 0 > r)) ca.remove(), ca = null
            } else {
                0 <= b && b <= X + 1 && (0 < B && 0 < y && b + l <= X + 1 && (fa = d.line(z, [0, B + 1], [b + l, b + l], A, y, C), x.push(fa)), 0 < D && (Ba = d.line(z, [0, ma, xa + ma], [b, b + na, b + na], K, D, C, H), x.push(Ba)));
                ga = "end";
    ...
    d.AmGraph = d.Class({
        construct: function (a) {
            ...
            var m = this.useNegativeColorIfDown, p = this.width, q = this.height, n = this.y, t = this.rotate,
                ...
                A, y, B, D, C = this.chart.graphs.length, K, H = this.dx / this.tcc, Q = this.dy / this.tcc,
                            ...
                            if (!isNaN(W)) {
                                ...
                                if (!(W < Qb && ob < Qb || W > Rb && ob > Rb)) {
                                    ...
                                    if (t) {
                                        ...
                                        J = d.roundTo(J, 2);
                                        ba = d.roundTo(ba, 2);
                                        S < p + Z * H && 0 < J && (this.showOnAxis && (R -= Q / 2), va = new d.Cuboid(h, J, ba, H - a.d3x, Q - a.d3y, Na, Va, da, Ca, this.lineAlpha, ga, w, t, ea, bb, mb, aa), v.columnHeight =
        ...
        construct: function (a) {
            ...
            if (!isNaN(a)) {
                ...
                if (this.show) {
                    ...
                    p.innerHTML = '<div style="text-align:' + this.textAlign + "; " + C + "max-width:" + this.maxWidth + "px; font-size:" + y + "px; color:" + this.color + "; font-family:" +
                        A + '">' + this.text + "</div>";
                    c.chartDiv.appendChild(p);
                    this.textDiv = p;
                    var K = p.offsetWidth, H = p.offsetHeight;
                    ...
                    this.drop ? (g = C / 1.6, l = 0, "V" == x && (x = "down"), "H" == x && (x = "left"), "down" == x && (D = a + 1, y = b - g - g / 3), "up" == x && (l = 180, D = a + 1, y = b + g + g / 3), "left" == x && (l = 270, D = a + g + g / 3 + 2, y = b), "right" == x && (l = 90, D = a - g - g / 3 + 2, y = b), k = y - H / 2 + 1, m = D - K / 2 - 1, q = d.drop(h, g, l, q, w, r, n, aa)) : 0 < u || 0 === z ? (0 < M && (a = d.rect(h, C, A, q, 0, r +
                    ...
                    0 < this.animationDuration && !e && !isNaN(this.prevX) && (f.translate(this.prevX, this.prevY, NaN, !0), f.animate({translate: D - h + "," + y}, b, "easeOutSine"), p && (B.left = this.prevTX + "px", B.top = this.prevTY + "px", this.xAnim = c.animate({node: p}, "left", this.prevTX, m, b, "easeOutSine",
                        "px"), this.yAnim = c.animate({node: p}, "top", this.prevTY, k, b, "easeOutSine", "px")));
                    this.prevX = D - h;
                    this.prevY = y;
                    this.prevTX = m;
                    this.prevTY = k
...
(function () {
    ...
    d.wedge = function (a, b, c, e, h, f, g, k, l, m, p, q, n, t) {
        ...
        e = a.set();
        var M;
        z && (x = r(10 * x), y = r(10 * y), D = r(10 * D), K = r(10 * K), A = r(10 * A), B = r(10 * B), C = r(10 * C), u = r(10 * u), b = r(10 * b), l = r(10 * l), c = r(10 * c), f *= 10, g *= 10, k *= 10, w *= 10, 1 > Math.abs(h) && 1 >= Math.abs(D - y) && 1 >= Math.abs(C - B) && (M = !0));
        h = "";
        var P;
        q && (H["fill-opacity"] = 0, H["stroke-opacity"] = m["stroke-opacity"] / 2, H.stroke = m.stroke);
        if (0 < l) {
            P = " M" + x + "," + (A + l) + " L" + y + "," + (B + l);
            z ? (M || (P += " A" + (b - f) + "," + (l + c - g) + "," + (b + f) + "," + (l + c + g) + "," + y + "," + (B + l) + "," + D + "," + (C + l)), P += " L" + K + "," + (u + l), 0 < k && (M || (P += " B" + (b - k) + "," + (l + c - w) + "," + (b + k) + "," +
        ...
        z ? (M || (h = " A" + r(b - f) + "," + r(c - g) + "," + r(b + f) + "," + r(c + g) + "," + r(y) + "," + r(B) + "," + r(D) + "," + r(C)), g = " M" + r(x) + "," + r(A) + " L" + r(y) + "," + r(B) + h + " L" + r(K) + "," + r(u)) : g = " M" + x + "," + A + " L" + y + "," + B + (" A" + f + "," + g + ",0," + Q + ",1," + D + "," + C) + " L" + K + "," + u;
        0 < k && (z ? M || (g += " B" + (b - k) + "," + (c - w) + "," + (b + k) + "," + (c + w) + "," + K + "," + u + "," + x + "," + A) : g += " A" + k + "," + w + ",0," + Q + ",0," + x + "," + A);
