(function () {
    ...
    e.AmSlicedChart = e.Class({
        inherits: e.AmChart, construct: function (a) {
            ...
            if (!a.hidden) {
                ...
                var h = this.formatString(this.balloonText, a, !0), k = this.balloonFunction;
                k && (h = k(a, h));
            ...
            !0 === c && (d = 0);
            if (c = a.wedge) 0 < d ? (c.animate({translate: b * a.pullX + "," + b * a.pullY}, d, this.pullOutEffect), a.labelSet && a.labelSet.animate({translate: b * a.pullX + "," + b * a.pullY}, d, this.pullOutEffect)) : (a.labelSet && a.labelSet.translate(b * a.pullX, b * a.pullY), c.translate(b * a.pullX, b * a.pullY));
            ...
            if (void 0 !== b) {
                var c = b.length, d = 0, f, h, k;
                for (f = 0; f < c; f++) {
                    h = {};
                    var n = b[f];
                    h.dataContext = n;
                    null !==
                    n[this.valueField] && (h.value = Number(n[this.valueField]));
            ...
            for (c = 0; c < a.length; c++) d = this.chartData[c], d.percents = !d.hidden &&
            ...
            for (c = 0; c < a.length; c++) {
                var d = a[c], f = this.formatString(this.labelText, d), h = this.labelFunction;
                h && (f = h(d, f));
...
(function () {
    ...
    e.AmFunnelChart = e.Class({
        inherits: e.AmSlicedChart, construct: function (a) {
            this.type = "funnel";
            ...
            if (e.ifArray(a)) if (0 < this.realWidth && 0 < this.realHeight) {
                var b = Math.round(this.depth3D * Math.cos(this.angle * Math.PI / 180)),
                    c = Math.round(-this.depth3D * Math.sin(this.angle * Math.PI / 180)), d = this.container,
                    f = this.startDuration, h = this.rotate, k = this.updateWidth();
                this.realWidth = k;
                var n = this.updateHeight();
                this.realHeight = n;
                var l = e.toCoordinate, t = l(this.marginLeft, k), u = l(this.marginRight, k),
                    g = l(this.marginTop, n) + this.getTitleHeight(), l = l(this.marginBottom, n);
                0 < b && 0 > c && (this.neckHeight = this.neckWidth =
                    0, h ? l -= c / 2 : g -= c / 2);
                var u = k - t - u, E = e.toCoordinate(this.baseWidth, u), I = e.toCoordinate(this.neckWidth, u),
                    D = n - l - g, F = e.toCoordinate(this.neckHeight, D), y = g + D - F;
                h && (g = n - l, y = g - D + F);
                this.firstSliceY = g;
                ...
                for (var z = u / 2 + t, G = (D - F) / ((E - I) / 2), C = 1, v = E / 2, E = (D - F) * (E + I) / 2 + I * F, H = g, M = 0, F = 0; F < a.length; F++) {
                    ...
                    if (!0 !== m.hidden && (this.showZeroSlices || 0 !== m.percents)) {
                        var A = [], q = [], p;
                        if ("height" == this.valueRepresents) p = D * m.percents / 100; else {
                            var B = -E * m.percents / 100 / 2, K = v;
                            p = -1 / (2 * G);
                            var r =
                                Math.pow(K, 2) - 4 * p * B;
                            0 > r && (r = 0);
                            p = (Math.sqrt(r) - K) / (2 * p);
                            if (!h && g >= y || h && g <= y) p = 2 * -B / I; else if (!h && g + p > y || h && g - p < y) w = h ? Math.round(p + (g - p - y)) : Math.round(p - (g + p - y)), r = w / G, B = 2 * (-B - (K - r / 2) * w) / I, Infinity != B && (p = w + B)
                        }
                        B = v - p / G;
                        K = !1;
                        ...
                        w = d.set();
                        0 < b && 0 > c ? (q = B / v, A = -1, h ||
                        (A = 1), isNaN(C) && (C = 0), A = (new e.Cuboid(d, 2 * v, A * p, b, c * C, m.color, m.alpha, this.outlineThickness, this.outlineColor, this.outlineAlpha, 90, 0, !1, 0, m.pattern, q)).set, A.translate(z - v, g - c / 2 * C), C *= q) : A = e.polygon(d, A, q, m.color, m.alpha, this.outlineThickness, this.outlineColor, this.outlineAlpha);
                        ...
                        if (q = this.gradientRatio) {
                            var r = [], x;
                            for (x = 0; x < q.length; x++) r.push(e.adjustLuminosity(m.color,
                                q[x]));
                        ...
                        m.ty0 = h ? g - p / 2 : g + p / 2;
                        this.labelsEnabled && this.labelText && m.percents >= this.hideLabelsPercent && (q = this.formatString(this.labelText, m), (A = this.labelFunction) && (q = A(m, q)), r = m.labelColor, r || (r = this.color), A = this.labelPosition, x = "left", "center" == A && (x = "middle"), "left" == A && (x = "right"), "" !== q && (q = e.wrappedText(d, q, r, this.fontFamily,
                            ...
                            m.hitRect = H, M = q.getBBox().height, H = x));
            ...
            for (var c = 0, d = this.chartData, f = d.length, e, k = 0; k < f; k++) {
                e = d[f - k - 1];
                var n = e.label, l = e.labelY, t = e.labelX, u = e.labelHeight, g = l;
...
(function () {
    ...
    e.Cuboid = e.Class({
        construct: function (a, b, c, d, e, h, k, n, l, t, u, g, E, I, D, F, y) {
            ...
            var a = this.set;
            a.clear();
            var b = this.container, c = b.chart, d = this.w, f = this.h, h = this.dx, k = this.dy, n = this.colors,
                l = this.alpha, t = this.bwidth, u = this.bcolor, g = this.balpha, E = this.gradientRotation,
                I = this.cornerRadius, D = this.dashLength, F = this.pattern, y = this.topRadius, z = this.bcn, G = n,
                C = n;
            "object" == typeof n && (G = n[0], C = n[n.length - 1]);
            var v, H, M, m, w, A, q, p, B, K = l;
            F && (l = 0);
            var r, x, J, L, N = this.rotate;
            if (0 < Math.abs(h) || 0 < Math.abs(k)) if (isNaN(y)) q = C, C = e.adjustLuminosity(G, -.2), C = e.adjustLuminosity(G, -.2), v = e.polygon(b, [0, h, d + h, d, 0], [0, k, k, 0, 0],
                ...
                var O, P, Q;
                N ? (O = f / 2, C = h / 2, Q = f / 2, P =
                    d + h / 2, x = Math.abs(f / 2), r = Math.abs(h / 2)) : (C = d / 2, O = k / 2, P = d / 2, Q = f + k / 2 + 1, r = Math.abs(d / 2), x = Math.abs(k / 2));
                J = r * y;
                L = x * y;
            ...
            d = isNaN(y) ? 0 > f ? [v,
                B, H, M, m, w, A, q, p, b] : [q, p, H, M, m, w, v, B, A, b] : N ? 0 < d ? [v, b, q] : [q, b, v] : 0 > f ? [v, b, q] : [q, b, v];
            e.setCN(c, b, z + "front");
            e.setCN(c, H, z + "back");
            e.setCN(c, q, z + "top");
            ...
            e.setCN(c, m, z + "left");
            e.setCN(c, w, z + "right");
            for (v = 0; v < d.length; v++) if (H = d[v]) a.push(H), e.setCN(c, H, z + "element");
