(function () {
    ...
    b.AmGanttChart = b.Class({
        inherits: b.AmSerialChart, construct: function (a) {
            this.type = "gantt";
            ...
            var z = this.categoryField, C = this.startField, D = this.endField, E = this.durationField,
                F = this.startDateField, G = this.endDateField, w = this.colorField, q = b.extractPeriod(this.period),
                f = q.period, q = q.count, r = b.getDate(this.startDate, this.dataDateFormat, "fff");
            ...
            if (v) for (a = 0; a < v.length; a++) {
                var e = v[a], l = {};
                l[z] = e[z];
                var x = e[this.segmentsField], t;
                this.ganttDataProvider.push(l);
                e = e[w];
                this.colors[a] || (this.colors[a] = b.randomColor());
                if (x) for (var g = 0; g < x.length; g++) {
                    var d = x[g], c = d[C], h = d[D], m = d[E];
                    isNaN(c) && (c = t);
                    isNaN(m) || (h = c + m);
                    var m = "start_" + a + "_" + g, y = "end_" + a + "_" + g;
                    l[m] =
                        c;
                    l[y] = h;
